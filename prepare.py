...
def config_logger(name, log_file=None):
    ...
    while True:
        ...
        for i, kern in enumerate(kern_chunks):
            ...
            if (process.returncode != 0):
                logger.error(f"tiefix error={process.returncode} on {chunk_path}")
            ...
            try:
                ...
                logger.exception(f"Exception while loading {chunk_path} audio. Reason: {e}")
                audio_errors += 1
                continue
            ...
            try:
                ...
                logger.exception(f"Discarded {chunk_path} due to error in kern sequence conversion. Reason {e}")
                krnseq_errors += 1
                continue

            if krnseq is None:
                logger.warning(f"Discarded {chunk_path} for double dots/sharps/flats")
            ...
            try:
                ...
                logger.warning(f"Discarded {chunk_path} during label encoding. Reason: {e}")
                encoding_errors += 1
                continue
            ...
            if duration > args.max_duration or duration < seqlen * args.min_duration_symbol:
                logger.warning(f"Sequence too long in {chunk_path} len={seqlen} duration={duration:.2f}")
                length_errors += 1
                continue
...
def process_scores(scores, args, labels):
    ...
    pool = mp.Pool(args.num_workers, initializer=process_sample, initargs=(q, samples, args, labels, invalid_counters))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--label-encoder', type=str, default='multi', choices=['simple', 'multi', 'multi_ext', 'multi2', 'multi2_ext'], help="type of encoder. Choose from 'simple', 'multi', 'multi_ext', 'multi2', 'multi2_ext'. Use multichar labels to reduce sequence size")
    parser.add_argument('--remove-splits', action="store_true", default=False, help="Clean splits and chords during kern cleaning")
    args = parser.parse_args()

    data_prep_job = f'{args.id}_{args.label_encoder}{"_splits" if not args.remove_splits else ""}'
    outdir = Path(args.out_dir) / data_prep_job
    outdir.mkdir(parents=True, exist_ok=True)
    args.out_dir = str(outdir)

    # Logging config.
    log_file = outdir / f'{datetime.now().strftime("%Y%m%d-%H%M%S")}_{data_prep_job}.log'
    config_logger('data_prep', log_file)
    ...
    logger.info("Number of train samples: {} ({:.2f} hours)".format(len(x_train), train_dur / 3600))
    logger.info("Number of validation samples: {} ({:.2f} hours)".format(len(x_val), val_dur / 3600))
    logger.info("Number of test samples: {} ({:.2f} hours)".format(len(x_test), test_dur / 3600))
    total_samples = len(x_train) + len(x_val) + len(x_test)
    total_dur = train_dur + val_dur + test_dur
    logger.info("Total samples: {} ({:.2f} hours)".format(total_samples, total_dur / 3600))
    ...
    logger.info("Creating label JSON file with {} symbols".format(len(labels.labels)))
